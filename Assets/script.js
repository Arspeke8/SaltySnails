...
var airQualitySearchTableCellEl = document.querySelector(
  "#air-quality-search-items"
);
...
function renderChecklist() {
  ...
  for (var i = 0; i < checklistArray.length; i++) {
    ...
    checklistItem.innerHTML =
      "<input type='checkbox' id='individualCheckbox' data-index='" +
      i +
      "'>" +
      checklistItemText;
...
function retrieveStations(cityName) {
  var airQualityStationSearchURL =
    "http://api.waqi.info/search/?keyword=" +
    cityName +
    "&token=" +
    airQualityAPIToken;
...
function renderSearchResults(searchResults) {
  for (var i = 0; i < searchResults.length; i++) {
    ...
    airQualitySearchCellEl.innerHTML =
      "<td>" +
      searchResults[i].station.name +
      "</td><td>" +
      searchResults[i].time.stime +
      "</td>";
...
function renderAirQuality(airQuality) {
  ...
  stationNameEl.textContent =
    "Station Name: " + airQuality.name + " on " + airQuality.date.s;
  ...
  co.innerHTML =
    "<th>Carbon Monoxide</th><td>" + airQuality.iaqi.co.v + "</td>";
  ...
  pressure.innerHTML =
    "<th>Atmospheric Pressure</th><td>" + airQuality.iaqi.p.v + "</td>";
...
function getApi() {
  var requestUrl =
    "https://api.data.charitynavigator.org/v2/organizations?app_id=503ffc66&app_key=d75996a1f4f75a04e4644d92b1467e8f&search=environment&pageSize=15";
